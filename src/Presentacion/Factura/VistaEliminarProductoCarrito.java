...
public class VistaEliminarProductoCarrito extends JFrame implements IGUI {
	...
	public VistaEliminarProductoCarrito() {
		vEliminarProductoCarrito();
	}

	public void vEliminarProductoCarrito() {
		setMinimumSize(new Dimension(500, 360));
		setTitle("Quitar Producto a Carrito");
		setIconImage(Toolkit.getDefaultToolkit().getImage(VistaAltaTarea.class.getResource("/icons/factura/quitar_producto_carrito-removebg-preview.png")));
		...
		JLabel logo = new JLabel("      Quitar Producto a Carrito");
		logo.setFont(new Font("Tahoma", Font.BOLD, 25));
		logo.setIcon(new ImageIcon(new ImageIcon(VistaAltaTarea.class.getResource("/icons/factura/quitar_producto_carrito-removebg-preview.png")).getImage().getScaledInstance(50, 50, java.awt.Image.SCALE_SMOOTH)));
		titlepanel.add(logo);
	...
	private void ok() {
		...
		Controlador.getInstance().update(Eventos.EliminarProductoCarrito, pair);
		if (cerrar) 
			dispose();
		else
			cerrar = true;
	...
	public void update(int event, Object object) {
		...
		case Eventos.EliminarProductoCarritoOk:
			JOptionPane.showMessageDialog(null, "�xito eliminando producto");
			Controlador.getInstance().update(Eventos.MainWindowFactura, null);
			break;
