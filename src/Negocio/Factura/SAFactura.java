...
public class SAFactura implements ISAFactura {
	...
	public Integer anyadirProductoaCarrito(Integer IDProducto, Integer cantidad) {
		...
		if (prd.getIdproyecto() == -1 || !prd.getActivo()) {
			System.out.println(
					"No se ha podido anyadir el producto al carrito (no existe o esta desactivado) - SAFactura");
			return -1;
		} else {
	...
	public Integer eliminarProductodeCarrito(Integer IDProducto, Integer cantidad) {
		...
		if (prd.getIdproyecto() == -1 || !prd.getActivo()) {
			System.out.println(
					"No se ha podido eliminar el producto del carrito (no existe o esta desactivado) - SAFactura");
			return -1;
		} else {
			...
			for (TLineaFactura s : set) {
				if (s.getIdProducto() == IDProducto) {
					...
					if (s.getCantidad() < cantidad) {
						System.out.println(
								"No se ha eliminado el producto del carrito (la cantidad a eliminar es mayor que la cantidad en el carrito) - SAFactura");
						return -1;
